<!--- **********************************************************
    Component  : dao.cfc (PostgreSQL Specific)
    Author    : Abram Adams
    Date    : 1/2/2007
    @version 0.0.68
       @updated 3/31/2015
    Description  : Targeted database access object that will
    controll all PostgreSQL specific database interaction.
    This component will use PostgreSQL syntax to perform general
    database functions.
...
<cfcomponent output="false" accessors="true" implements="IDAOConnector">
  ...
  <cffunction name="delete" hint="I delete records from the database.  I am PostgreSQL specific." returntype="boolean" output="false">
  ...
  <cffunction name="deleteAll" hint="I delete all records from the passed tablename.  I am PostgreSQL specific." returntype="boolean" output="false">
    <cfargument name="TableName" required="true" type="string" hint="Table to delete from.">
  ...
  <cffunction name="select" hint="I select records from the database. I am PostgreSQL specific." returntype="query" output="false">
    ...
    <!--- <cftry> --->
      <cfif listlen(trim(arguments.sql), ' ') GT 1>
        <cfif len(trim(arguments.cachedwithin))>
          <cfquery name="__get" datasource="#getDsn()#" cachedwithin="#arguments.cachedwithin#">
      ...
      <cfelse>
        ...
        <cfelse>
          <cfset var querySQL = "SELECT #arguments.columns# FROM #arguments.table#" />
          <cfset var queryParams = [] />
          <cfset var queryOptions = { datasource = "#getDsn()#" } />
...
<cfdump var="#tmpSQL#" /><cfabort />

            <cfset var statement = 0 />

            <cfloop array="#tmpSQL.statements#" index="statement">
              <!--- #tmpSQL.statements[idx].before# --->
              <cfif structKeyExists( statement, 'cfsqltype' )>
                <cfset arrayAppend( queryParams, {
                  cfsqltype = statement.cfSQLType,
                  value = statement.value,
                  list = statement.isList
                } ) />
              </cfif>
            </cfloop>
          <!--- /Parse out the queryParam calls inside the where statement --->
          ...
          <cfif val( arguments.limit ) GT 0>
            <cfset querySQL &= "LIMIT #val( arguments.limit )#" />
          </cfif>

          <cfif len( trim( arguments.orderby ) )>
            <cfset querySQL &= "ORDER BY #arguments.orderby#" />
          </cfif>

<cfdump var="#querySQL#" /><cfabort />

          <!--- <cfset __get = queryExecute( querySQL, queryParams, queryOptions ) /> --->
        </cfif>
      </cfif>
  ...
  <cffunction name="write" hint="I insert data into the database.  I take a tabledef object containing the tablename and column values. I return the new record's Primary Key value.  I am PostgreSQL specific." returntype="any" output="false">
  ...
  <cffunction name="update" hint="I update all fields in the passed table.  I take a tabledef object containing the tablename and column values. I return the record's Primary Key value.  I am PostgreSQL specific." returntype="any" output="false">
    ...
    <cftry>
      ...
      <cfcatch type="any">
        ...
        <cfthrow errorcode="803-postgres.update" type="dao.custom.error" detail="#lp.getGlobalLabel('Unexpected Error')#" message="#lp.getGlobalLabel('There was an unexpected error updating the database.  Please contact your administrator')#. #cfcatch.message#">
  ...
  <cffunction name="define" hint="I return the structure of the passed table.  I am PostgreSQL specific." returntype="any" output="false">
    ...
    <cfquery name="def" datasource="#getDsn()#">
      ...
      WHERE     a.attnum > 0
        ...
        AND     pgc.relname = <cfqueryparam cfsqltype="cf_sql_varchar" value="#TableName#" />
  ...
  <cffunction name="getPrimaryKey" hint="I return the primary key column name and type for the passed in table.  I am PostgreSQL specific." returntype="struct" output="false">
    ...
    <cfquery dbtype="query" name="get">
      SELECT field, type FROM def WHERE pk = 1
    </cfquery>
  ...
  <cffunction name="getPrimaryKeys" hint="I return the primary keys column name and type for the passed in table.  I am PostgreSQL specific." returntype="array" access="public" output="false">
    <cfargument name="TableName" required="true" type="string" hint="Table to return primary key.">
    ...
    <cfoutput query="get">
      <cfset arrayAppend( ret, {
        "field" = field,
        "type" = getDAO().getCFSQLType( listFirst( get.type, '(' ))
      }) />
    </cfoutput>
    ...
    * */
    ...
    public tabledef function makeTable( required tabledef tabledef ) output = false {
      ...
      for ( var colName in tableDef.getTableMeta().columns ){
        ...
        col.name = colName;
        var safeColname = "#getSafeIdentifierStartChar()##col.name##getSafeIdentifierEndChar()#";

        switch( col.sqltype ){
          case 'string':
            tmpstr = '#safeColname# varchar(#structKeyExists( col, 'length' ) ? col.length : '255'#) #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & col.default & "'": ''#';
          break;
          case 'numeric':
            tmpstr = '#safeColname# #structKeyExists(col,'generator') && col.generator eq 'increment' ? 'serial' : 'int'# #col.isPrimaryKey ? ' PRIMARY KEY' : ''# #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & col.default & "'": ''#';
          break;
          case 'date':
            tmpstr = '#safeColname# timestamp(3) #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & col.default & "'": ''#';
          break;
          case 'tinyint':
            tmpstr = '#safeColname# #structKeyExists(col,'generator') && col.generator eq 'increment' ? 'serial' : 'tinyint(1)'# #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & (col.default ? 1 : 0) & "'": ''#';
          break;
          case 'boolean': case 'bit':
            tmpstr = '#safeColname# boolean #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT " & (col.default ? "'t'" : "'f'") : ''#';
          break;
          case 'text':
            tmpstr = '#safeColname# text #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & (col.default ? 1 : 0) & "'": ''#';
          break;
          default:
            tmpstr = '#safeColname# #col.type# #structKeyExists( col, 'length' ) ? '(' & col.length & ')' : '(255)'# #( col.isPrimaryKey || col.isIndex ) ? 'NOT' : ''# NULL #structKeyExists(col,'default') ? "DEFAULT '" & col.default & "'": ''#';
          break;
        }
        ...
        if( col.isPrimaryKey ){
          if( structKeyExists( col, 'generator' ) && col.generator eq 'increment' ){
            primaryKeys = listPrepend(primaryKeys, '#safeColname#');
          }else{
            primaryKeys = listAppend(primaryKeys, '#safeColname#');
          }
        }
...
writeDump(tableSQL);abort;
    ...
    public void function dropTable( required string table ) output = false{
      getDao().execute( 'DROP TABLE IF EXISTS #getSafeIdentifierStartChar()##table##getSafeIdentifierEndChar()#' );
    }
  </cfscript>
</cfcomponent>